...
final class TrackerStore: NSObject, NSFetchedResultsControllerDelegate {
    ...
    func createTracker(from tracker: TrackerCoreData) throws -> Tracker {
        ...
        let color = ColorConvert.convertStringToColor(hex: colorHEX)
        let timetable = Schedule(markedDays: DaysOfWeek.reversTransformedSked(value: tracker.timetable)!)
        return Tracker(id: id, name: name, color: color!, emoji: emoji, timetable: timetable, daysCount: daysCount.count)
    }
    ...
    func currentlyTrackers(date: Date, searchString: String) throws {
        ...
        predicates.append(NSPredicate(
            ...
            #keyPath(TrackerCoreData.timetable),
            #keyPath(TrackerCoreData.timetable),
            #keyPath(TrackerCoreData.timetable), matching
        ))
...
extension TrackerStore: TrackerStoreProtocol {
    ...
    func addTracker(tracker: Tracker, category: TrackerCategory) throws {
        ...
        trackerData.color = ColorConvert.convertColorToString(color: tracker.color)
        trackerData.timetable = Schedule(markedDays: DaysOfWeek.reversTransformedSked(value: tracker.timetable?.scheduleText)!).scheduleText
